...
#mnist数据集相关常数
INPUT_NODE = 784   #输入层节点数，对于mnist来说等于图片像素
...
def train(mnist):
    ...
    regularizer = tf.contrib.layers.l2_regularizer(REGULARIZATION_RATE)  #定义的是函数名字
    regularization = regularizer(weights1) + regularizer(weights2)
    loss = cross_entropy_mean + regularization
    learning_rate = tf.train.exponential_decay(LEARNING_RATE_BASE, global_step, mnist.num_examoles / BATCH_SIZE, LEARNING_RATE_DECAY)
